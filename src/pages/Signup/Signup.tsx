...
export default function Signup() {
    function isValuesValid(touched: FormikTouched<{
        fullname: string;
        email: string;
        password: string;
        confirm: string;
        ...
        // let isValid = Object.(touched).reduce( (prev, curr) => prev || curr, false)
        let isValid = false;

        if(Object.keys(touched).length == 0) return true

        for(const key in errors){         
            isValid = isValid || !touched[key as keyof FormikTouched<{ fullname: string; email: string; password: string; confirm: string; }>] || (!!touched[key as keyof FormikTouched<{ fullname: string; email: string; password: string; confirm: string; }>] && !!errors[key as keyof FormikErrors<{ fullname: string; email: string; password: string; confirm: string; }>]);
        }
            ...
            <div className="flex flex-col h-full justify-evenly gap-8 py-4">
                ...
                <div className="flex flex-col">
                    <Formik
                                    ...
                                    switch (input) {
                                        ...
                                        case 'confirm':
                                            if(values.password && values.confirm && values.password != values.confirm){
                                                error.password = "Password and password confirmation does not match"
                                                error.confirm = "Password and password confirmation does not match"
                                            }
                                            break;
                        ...
                        onSubmit={(values: { fullname: string, email: string, password: string, confirm: string }, formikHelpers: FormikHelpers<{ fullname: string, email: string, password: string, confirm: string }>) => {
                            setTimeout(() => {
                ...
                <p className="w-full text-center">
                    ...
                    <a href="#" className="text-c-yellow font-semibold markazi text-2xl">Sign in</a>
                </p>
